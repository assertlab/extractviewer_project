 ...
 * 
 * @author helaine.lins
 * @created 04/12/2014 - 10:44:49
 */
public class ControladorArquivosBibtex {

	/**
	 * Representa o mecanismo de log da classe.
	 */
	private static Logger LOG = Logger
			.getLogger(ControladorArquivosBibtex.class.getName());

	/**
	 * Executa o diagnóstico da leitura do arqvuivo de bibitex
	 * 
	 * @param nome O nome do arquivo a ser diagnosticado.
	 */
	public void executarDiagnosticoBibTexArquivosSelecionados(String nome) {
		try {
			List<Estudo> estudos = this.recuperarEstudosScienceDirect(nome, true);
	 ...
	 * @param nome O nome do arquivo a ser diagnosticado.
	 */
	public void tratarBibTexArquivosSelecionados(String nome) {

		// carregar o bibtex
		List<Estudo> estudos = this.recuperarEstudosScienceDirect(nome, false);

		// gerar planilha igual a do reviewer, para análise posterior de
		// arquivos duplicados.
		this.preencherPlanilha(nome, estudos);
	 ...
	 * 
	 * @param nome O nome do arquivo a ser recuperado.
	 * @param diagnostico Indica se a leitura deve ser feita no modo diagnóstico.
	 * 
	 * @return Um {@link List} de {@link Estudo} contendo os dados dos estudos
	 *         que constam no arquivo mapeamento-ese-manual.bib.
	 */
	private List<Estudo> recuperarEstudosScienceDirect(String nome, boolean diagnostico) {
		List<Estudo> estudos = new ArrayList<Estudo>();

		Collection<BibTeXEntry> registros = this
				.recuperarRegistrosArquivoBibTex(nome);
		...
		try {
			...
			LOG.error("Ocorreu um erro ao obter os estudos do arquivo .bib", e);

			if (!diagnostico) {
				throw new MapeamentoException(
						"Ocorreu um erro ao obter os estudos do arquivo .bib",
						e);
			}
		}
	 ...
	 *             arquivo;
	 */
	private Collection<BibTeXEntry> recuperarRegistrosArquivoBibTex(String nome) {
		...
		try {
			...
			Reader reader = new InputStreamReader(
					new FileInputStream(
							MapeamentoUtil
									.recuperarMensagemProperties(MapeamentoUtil.CHAVE_DIRETORIO_BIB)
									+ "/" + nome));
	 ...
	 */
	private void preencherPlanilha(String nome, List<Estudo> estudos) {
		try {
			...
			if (!estudos.isEmpty()) {
				...
				FileOutputStream fileOut = new FileOutputStream(
						MapeamentoUtil
								.recuperarMensagemProperties(MapeamentoUtil.CHAVE_DIRETORIO_DESTINO_BIB)
								+ "/" + nome + ".xls");
